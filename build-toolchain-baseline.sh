...
# Sets up a toolchain without FineIBT to serve as baseline.
...
TOOLCHAIN_BASE="${CURDIR}/toolchains/baseline"
...
build_crt() {
    ...
    cmake \
        ...
        -DCMAKE_C_FLAGS="--rtlib=libgcc -Wl,-znow" \
        -DCMAKE_CXX_FLAGS="--rtlib=libgcc -Wl,-znow" \
...
build_libunwind() {
    ...
    cmake \
        ...
        -DCMAKE_C_COMPILER="${INSTALL_LLVM}/bin/clang" \
        -DCMAKE_CXX_COMPILER="${INSTALL_LLVM}/bin/clang++" \
        -DCMAKE_C_FLAGS="-Wl,-znow" \
        -DCMAKE_CXX_FLAGS="-Wl,-znow"
    cmake --build "${bdir}"
    cmake --install "${bdir}"
}
